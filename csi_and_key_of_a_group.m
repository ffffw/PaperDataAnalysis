...
plotData = data;
for i = 1: size(data, 2)
    plotData(:, i) = data(:, i) * normScale / max(data(:, i));
end
...
bitLen = 50;
N = 2;
text(270, sum(ylim) - 2, '将信道频率响应量化成3阶，然后转换成格雷码', 'horiz', 'left', 'fontWeight', 'bold', 'fontSize', fontSize)
text(110, sum(ylim) - N - 2, ['Key generated by Alice: ' keyAlice(1:bitLen)], 'horiz', 'left', 'fontWeight', 'bold', 'fontSize', fontSize)
text(110, sum(ylim) - N - 3, ['Key generated by Bob  : ' keyBob(1:bitLen)], 'horiz', 'left', 'fontWeight', 'bold', 'fontSize', fontSize)
text(110, sum(ylim) - N - 4, ['Key generated by Eve  : ' keyEve(1:bitLen)], 'horiz', 'left', 'fontWeight', 'bold', 'fontSize', fontSize)
